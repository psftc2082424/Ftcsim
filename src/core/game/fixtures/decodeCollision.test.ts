...
describe('DECODE collision classification', () => {
  ...
  it('models the triangular GOAL shell and raised classifier channel', () => {
    ...
    expect(field.bodies[4]?.shape.kind).toBe('poly');
    expect(field.bodies[6]?.shape.kind).toBe('obb');
  });

  it('blocks an artifact at a solid GOAL shell', () => {
    const field = createDecodeField();
    const blueGoal = field.bodies[5];
    if (blueGoal === undefined || blueGoal.shape.kind !== 'poly') throw new Error('blue GOAL triangle missing');

    const artifact = createCircle(inchesToMeters(4.9) / 2);
    const point = blueGoal.shape.vertices.reduce(
      (sum, vertex) => vec2(sum.x + vertex.x / 3, sum.y + vertex.y / 3),
      vec2(0, 0),
    );
    const contact = collide(artifact, { p: point, theta: 0 }, blueGoal.shape, blueGoal.pose);

    expect(contact).not.toBeNull();
    expect(contact?.depth ?? 0).toBeGreaterThan(0);
    expect(blueGoal.span.top).toBeCloseTo(inchesToMeters(GOAL.heightIn.value), 12);
  });

  it('resolves a loose artifact out of the solid shell in the live world', () => {
    const field = createDecodeField();
    const blueGoal = field.bodies[5];
    if (blueGoal === undefined || blueGoal.shape.kind !== 'poly') {
      throw new Error('blue GOAL triangle missing');
    }

    // Start deliberately overlapping the triangle.  The world must
    // use the same static fixture body for a piece as it does for a robot.
    const start = blueGoal.shape.vertices.reduce(
      (sum, vertex) => vec2(sum.x + vertex.x / 3, sum.y + vertex.y / 3),
      vec2(0, 0),
    );
  ...
  it('uses the same GOAL fixture to exclude a robot', () => {
    const field = createDecodeField();
    const blueGoal = field.bodies[5];
    if (blueGoal === undefined || blueGoal.shape.kind !== 'poly') {
      throw new Error('blue GOAL triangle missing');
  ...
  it('keeps the physical tunnel opening passable below the raised classifier', () => {
    const field = createDecodeField();
    const classifier = field.bodies[6];
